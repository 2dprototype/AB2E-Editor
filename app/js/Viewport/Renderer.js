...
Renderer.prototype.renderJoint = function(joint, navigator){
	...
	if(joint.jointType == Joint.JOINT_AREA){
		...
		// joint.localAnchorA = joint.bodyA.position;
		// joint.localAnchorB = joint.bodyB.position;
	...
	if(joint.jointType == Joint.JOINT_REVOLUTE){
		...
		this.context.globalAlpha = this.JointAlpha;
		this.context.translate(joint.position[0], joint.position[1]);
		// this.context.rotate(joint.referenceAngle * Math.PI / 180);
	...
	if (joint.jointType == Joint.JOINT_MOTOR){
	...
	if (joint.jointType == Joint.JOINT_ROPE){
		...
		this.context.lineTo(joint.localAnchorA[0], joint.localAnchorA[1]);
		this.context.stroke();
		var size = joint.getFrequecyHZBounds()[2]/2;
	...
	if (joint.jointType == Joint.JOINT_REVOLUTE){
		...
		if(joint.inEditMode){
			...
			if (joint.enableLimit){
				...
				var x = localAnchor[0] + 100 * Math.cos((joint.lowerAngle * Math.PI / 180) + (angle  * Math.PI / 180));
				var y = localAnchor[1] + 100 * Math.sin((joint.lowerAngle * Math.PI / 180) + (angle  * Math.PI / 180));
				...
				this.context.beginPath();
				this.context.moveTo(localAnchor[0], localAnchor[1]);
				var x = localAnchor[0] + 100 * Math.cos((joint.upperAngle * Math.PI / 180) + (angle  * Math.PI / 180));
				var y = localAnchor[1] + 100 * Math.sin((joint.upperAngle * Math.PI / 180) + (angle  * Math.PI / 180));
				this.context.lineTo(x, y);
				this.context.stroke();
				this.context.closePath();
    ...
    if (joint.jointType == Joint.JOINT_WHEEL){
		//
		this.context.moveTo(joint.localAnchorA[0], joint.localAnchorA[1]);
		this.context.lineTo(joint.localAnchorB[0], joint.localAnchorB[1]);
		//localAnchorA to bodyA
    ...
    if (joint.jointType == Joint.JOINT_PRISMATIC){
		...
		if (joint.enableLimit){				
			x0 = joint.localAnchorA[0] + joint.localAxisA[0] * joint.lowerTranslation;
			y0 = joint.localAnchorA[1] + joint.localAxisA[1] * joint.lowerTranslation;
			x1 = joint.localAnchorA[0] + joint.localAxisA[0] * joint.upperTranslation;
			y1 = joint.localAnchorA[1] + joint.localAxisA[1] * joint.upperTranslation;
...
Renderer.prototype.renderImage = function(s, body){
	...
	if(points.length > 0){
		...
		this.context.globalAlpha = s.opacity / 100;
		this.context.scale(s.flip[0], s.flip[1]);
		if(s.sprite != null){
	...
	if(body.isSelected){
		if (body.sprites.length > 0){
			var s = body.sprites[body.selectedSprite];
			if(s.hasLoaded && (this.sceneManager.checkState(this.sceneManager.STATE_IMAGE_EDIT_MODE) || this.sceneManager.checkState(this.sceneManager.STATE_IMAGE_VERTEX_EDIT_MODE))){
				...
				if(points.length > 0){
					if(s.inEditMode){
						...
						this.context.beginPath();
						this.context.moveTo(points[0].x, points[0].y);
						this.renderVertex(points[0]);
						for(i = 1; i < points.length; i++){
							var p = points[i];
							this.context.lineTo(points[i].x, points[i].y);
							this.renderVertex(points[i]);
						}
						this.context.closePath();
						this.context.stroke();
					}
...
Renderer.prototype.renderBody = function(body, index){
	...
	if (body.isSelected){
		...
		if(this.DEBUG){
			...
			this.context.fillText(`Rotation : ${body.rotation.toFixed(2)}°`, body.bounds[0] - (body.bounds[2] / 2), body.bounds[1] - (body.bounds[3] / 2) - (3 / this.scale));
			this.context.fillText(`[${index}] : ${body.name}`, body.bounds[0] - (body.bounds[2] / 2), body.bounds[1] + (body.bounds[3] / 2) + (10 / this.scale));		    
		}

	}

	// draw axes of body
	if(this.bodyCenter) this.drawAxes(body.position[0], body.position[1], 25 / this.scale);
...
Renderer.prototype.renderParticles = function(p){
	...
	if(p.shape.type == 0){
		...
		this.context.lineTo(p.shape.radius, 0);
		this.context.stroke();
		this.renderCircle(0, 0, p.shape.radius, true);
		this.context.restore();
	}
		...
		this.context.translate(p.position[0], p.position[1]);
		this.context.rotate(p.rotation * Math.PI / 180);
		this.renderBox(0, 0, p.shape.width, p.shape.height, true);
		this.context.restore();
	}
	else if(p.shape.type == 2){
		this.context.beginPath();
	...
	if (p.isSelected){
		...
		this.context.lineWidth = 1 / this.scale;
		if(p.shape.type == 2 && p.shape.inEditMode){
			this.context.beginPath();
			this.context.strokeStyle = "#f00";
			this.context.moveTo(p.shape.vertices[0].x, p.shape.vertices[0].y);
...
Renderer.prototype.renderShape = function(shape, bodyColor){
	...
	if (shape.shapeType == Shape.SHAPE_CHAIN || shape.shapeType == Shape.SHAPE_EDGE){
		...
		this.context.stroke();
			...
			this.context.lineWidth = 1 / this.scale;
			this.context.moveTo(0, 0);
			this.context.lineTo(shape.radius, 0);
			this.context.stroke()
			this.context.lineWidth = 0;
			//circle
			this.context.beginPath();
			this.context.arc(0, 0, shape.radius, 0, 2 * Math.PI, false);
			this.context.fill();
			this.context.stroke();
			this.context.closePath();
	...
	// draw vtx if concave
	if (shape.shapeType == Shape.SHAPE_POLYGON){
		if (!shape.isConvex()){
			var polys = shape.decomposeToConvex(0, 0);
			for(i = 0; i < polys.length; i++){
				if (polys[i].vertices.length > 0){
					this.context.strokeStyle = bodyColor || this.dynamicBodyColor;
					this.context.beginPath();
					this.context.moveTo(polys[i].vertices[0][0] + polys[i].position[0], polys[i].vertices[0][1] + polys[i].position[1]);
					for(j = 1; j < polys[i].vertices.length; j++){
						var v = polys[i].vertices[j];
						this.context.lineTo(v[0] + polys[i].position[0], v[1] + polys[i].position[1]);
					}
					this.context.closePath();
					this.context.stroke();
				}
	...
	if(shape.isSelected) this.centroid(shape.centroid[0], shape.centroid[1], 12 / this.scale);
	if(this.bodyCenter) this.drawAxes(shape.position[0], shape.position[1], 25 / this.scale);
