...
b2Loader.prototype.reset = function(){
	if (this.loadSprites.length > 0){
		delete(this.loadSprites);
	}
	this.loadedBodies = [];	
...
b2Loader.prototype.loadScene = function(scene, world){
	...
	for (var i = 0; i < scene.bodies.length; i++){
		this.loadBody(scene.bodies[i], this.world);
	}
	// load joints
	for (var i = 0; i < scene.joints.length; i++){
		this.loadJoint(scene.joints[i], this.world);
	}
	// load particles
	for (var i = 0; i < scene.particles.length; i++){
		this.loadParticle(scene.particles[i], this.world);
	}
	// load sprites
	for (var i = 0; i < scene.sprites.length; i++){
		this.loadSprites(scene.sprites[i]);
	}
	
};


b2Loader.prototype.loadBody = function(obj, world){
	this.loadedBodies.push(this.createBody(obj, world));
}
	
b2Loader.prototype.loadJoint = function(obj, world){
	this.loadedJoints.push(this.createJoint(obj, world));
}
	
b2Loader.prototype.loadParticle = function(obj, world){
	this.loadedParticles.push(this.createParticle(obj, world));
}

b2Loader.prototype.loadSprites = function(sprite){
	var p = {};
	p.body = this.loadedBodies[sprite.body];
	p.zIndex = sprite.zIndex;
	var src = Editor.currentFile.dir + '//' + sprite.src
	...
	p.height = sprite.height;
	p.width = sprite.width;
	p.x = sprite.x;
	p.y = sprite.y;
	p.vertices = sprite.vertices;
	p.alpha = sprite.alpha;
	p.flip = sprite.flip;
	p.angle = sprite.rotation;
	p.rot = sprite.rot;
	
	this.loadedSprites.push(p);
}
...
b2Loader.prototype.createParticle = function(p, world){
	...
	return world.GetParticleSystemList().CreateParticleGroup(pd)
